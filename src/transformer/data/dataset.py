...
def collate_translation_batch(batch: list[tuple[torch.Tensor, torch.Tensor]], pad_id: int) -> dict[str, torch.Tensor]:
    ...
    tgt_batch_shifted = tgt_batch[:, :-1] # (batch, seq_len_tgt-1); decoder input is right shifted => has <BOS> but not <EOS>
    ...
    return {
        ...
        'src_pad_mask': get_pad_mask(src_batch, pad_id), # (batch, seq_len_src); whereever the tensor has pad_id, should be True
...
def drop_long_rows(src_tokens: list[list[int]], tgt_tokens: list[list[int]], max_seq_len: int) -> tuple[list[list[int]], list[list[int]]]:
    src_filtered = []
    tgt_filtered = []
...
def preprocess_data(src_data: list[str], tgt_data: list[str], src_tokenizer: Tokenizer, tgt_tokenizer: Tokenizer, max_seq_len: int) -> tuple[list[list[int]], list[list[int]]]:
...
def build_translation_dataset(raw: DatasetDict, src_lang: str, tgt_lang: str, max_seq_len: int, src_tokenizer: Tokenizer, tgt_tokenizer: Tokenizer) -> dict[str, TranslationDataset]:
    train_data = preprocess_data(raw['train'][src_lang], raw['train'][tgt_lang], src_tokenizer, tgt_tokenizer, max_seq_len)
    train_data = TranslationDataset(*train_data)

    val_data = preprocess_data(raw['validation'][src_lang], raw['validation'][tgt_lang], src_tokenizer, tgt_tokenizer, max_seq_len)
