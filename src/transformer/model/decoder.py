...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, x_enc: torch.Tensor, self_mask: torch.Tensor | None = None, cross_mask: torch.Tensor | None = None) -> torch.Tensor:
...
class Decoder(nn.Module):
    def __init__(self, n_dec: int, vocab_size: int, max_seq_len: int, d_model: int, d_kq: int, d_v: int, h: int, d_ff: int, p_drop: float=0.1) -> None:
        super().__init__()
        ...
        self.dec = nn.ModuleList([DecoderLayer(d_model=d_model, d_kq=d_kq, d_v=d_v, h=h, d_ff=d_ff, p_drop=p_drop) for _ in range(n_dec)])
