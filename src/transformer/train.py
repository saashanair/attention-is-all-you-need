...
def train(cfg: TrainConfig):
    ...
    data, src_vocab_size, tgt_vocab_size, pad_id = prepare_data(
        dataset_name=cfg.dataset_name,
        src_lang=cfg.src_lang,
        tgt_lang=cfg.tgt_lang,
        max_seq_len=ta_cfg.max_seq_len,
    ...
    test_loader = DataLoader(  # noqa: F841 -- wired up once test-set eval is implemented
        dataset=data['test'],
        batch_size=cfg.batch_size,
        shuffle=False,
        num_workers=0,
    ...
    tv_cfg = TransformerVocabConfig(
        src_vocab_size=src_vocab_size, tgt_vocab_size=tgt_vocab_size, shared_embeddings=cfg.shared_embeddings
    )
    ...
    optim = torch.optim.Adam(
        params=transformer.parameters(), lr=1.0, betas=(optim_cfg.beta1, optim_cfg.beta2), eps=optim_cfg.eps
    )
    lr_scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer=optim, lr_lambda=lambda step_num: get_lr(step_num, ta_cfg.d_model, optim_cfg.warmup_steps)
    )
    ...
    for epoch in range(cfg.num_epochs):
        ...
        with torch.no_grad():
            for vbatch_idx, vbatch in enumerate(tqdm(val_loader, desc='  val', unit='batch', leave=False)):
                vout = transformer(
                    src=vbatch['encoder_input'],
                    tgt=vbatch['decoder_input'],
                    src_pad_mask=vbatch['src_pad_mask'],
                    tgt_pad_mask=vbatch['tgt_pad_mask'],
                )
