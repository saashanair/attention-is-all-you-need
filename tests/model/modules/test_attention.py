...
class TestMultiHeadAttentionForward:
    ...
    @pytest.mark.parametrize("mask, perturb_keys", [
        pytest.param(_mask_1d([8, 9]),                   [8, 9], id="1d_perturb_masked"),  # bump the same keys the mask blocks -> output must not move
        pytest.param(_mask_1d([8, 9]),                   [0, 4], id="1d_perturb_visible"), # bump keys the mask lets through -> output must move
        pytest.param(_mask_2d([(0, 5), (1, 5), (2, 5)]), [5],    id="2d_per_query"),       # key 5 blocked for rows 0-2 only -> rows 0-2 immune, rows 3-4 not
    ])
    def test_mask_excludes_disallowed_keys(self, mha_layer, x_q, x_kv, mask, perturb_keys):
        ...
        full = mask.expand(SL_Q, SL_KV)       # (SL_Q, SL_KV) view; a 1d mask broadcasts up over queries, a 2d mask is unchanged
        perturb = torch.tensor(perturb_keys)  # the key columns we bump

        out1 = mha_layer(x_q=x_q, x_kv=x_kv, mask=mask)
        assert not out1.isnan().any()         # none of these masks block every key for a query, so no row softmaxes to all -inf -> NaN
