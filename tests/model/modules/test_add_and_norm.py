...
class TestAddAndNormForward:
    ...
    def test_dropout_in_train_mode(self, add_and_norm_layer, x_residual, x_sublayer):
        ...
        assert not torch.allclose(
            add_and_norm_layer(x_residual, x_sublayer), add_and_norm_layer(x_residual, x_sublayer)
        )
    ...
    def test_dropout_only_applied_to_sublayer(self, x_residual, x_sublayer):
        an_layer = AddAndNorm(
            d_model=D_MODEL, p_drop=1.0
        )  # special case: when p_drop=1, all neurons relating to the sublayer branch should be dropped
